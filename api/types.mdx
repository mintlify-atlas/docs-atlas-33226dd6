...
```typescript
import { createPreciseConverter } from 'univerto/core';
import { Rational } from 'univerto/core';

// Define a scale
const myScale = {
  small: new Rational(1, 1),
  large: new Rational(100, 1)
};

// Create a converter with inferred types
const MyConverter = createPreciseConverter(myScale);

// Type: 'small' | 'large'
type MyUnit = 'small' | 'large';

// TypeScript enforces valid units
MyConverter.from(10, 'small');  // ✓ Valid
MyConverter.from(10, 'medium'); // ✗ Type error
```
